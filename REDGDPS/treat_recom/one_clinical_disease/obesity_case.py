        ...
        if(current_hba1c>=target_):
            ...
            if previous_state == "Second time":
                ...
                if([("Metformin").upper()] == list(med_dose_last_time.keys())):
                    proposed_med[("Metformin").upper()]=full_dose
                    if(current_BMI>35):
                        proposed_med["GLP1RA"]=full_dose #Modified
                        proposed_med["Bariatric surgery".upper()]=""
                    else:
                        proposed_med[("SGLT2i or GLP1RA or dual GIP/GLP1ra").upper()]=full_dose
                ...
                # Here we'll work on the case of having "Bariatric surgery" in the recommended treatment
                bariatric_surgery_exists = False
                if "Bariatric surgery".upper() in med_dose_last_time:
                    bariatric_surgery_exists = True
                    del med_dose_last_time["Bariatric surgery".upper()]
                ...
                if(("Metformin").upper() in med_dose_last_time and ("SGLT2i").upper() in med_dose_last_time and len(med_dose_last_time)==2):
                    ...
                    if(("dual GIP/GLP1ra").upper() in med_dose_last_time):
                        proposed_med[("dual GIP/GLP1ra").upper()]=full_dose
                    else:
                ...
                if current_BMI>35 and bariatric_surgery_exists and len(proposed_med)>1:
                    proposed_med["Bariatric surgery".upper()]=""
        else:
